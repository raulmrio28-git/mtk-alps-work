...
//jdt force charging 20260301
    ...
    if (capacity >= 100) {
        ...
        LOG_ANIM("jdt [show_animation_common: %s %d]charging_animation_index = %d\n",__FUNCTION__,__LINE__, charging_animation_index); 
        static RECT_REGION_T battery_rect = {TOP_ANIMATION_LEFT,TOP_ANIMATION_TOP,TOP_ANIMATION_RIGHT,TOP_ANIMATION_BOTTOM};    

        fill_animation_logo(ANIM_V0_BACKGROUND_INDEX, fill_addr, dec_logo_addr, logo_addr,phical_screen);
        fill_animation_dynamic(BAT_ANIM_START_0 + charging_animation_index, battery_rect, fill_addr, percent_pic_addr, logo_addr, phical_screen);
